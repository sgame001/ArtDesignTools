...
public class CoroutineAdapter : CrossBindingAdaptor
{
    ...
    internal class Adaptor : IEnumerator<System.Object>, IEnumerator, IDisposable, CrossBindingAdaptorType
    {
        ...
        AdaptorMethodRecorder disposeRecorder = new AdaptorMethodRecorder();
        public void Dispose()
        {
            if (!disposeRecorder.mMethodGot)
            {
                disposeRecorder.mMethod = instance.Type.GetMethod("Dispose", 0);
                if (disposeRecorder.mMethod == null)
                {
                    disposeRecorder.mMethod = instance.Type.GetMethod("System.IDisposable.Dispose", 0);
                }
                disposeRecorder.mMethodGot = true;
            }

            if (disposeRecorder.mMethod != null)
            {
                appdomain.Invoke(disposeRecorder.mMethod, instance, null);
            }
        }

        AdaptorMethodRecorder moveNextRecorder = new AdaptorMethodRecorder();
        public bool MoveNext()
        {
            if (!moveNextRecorder.mMethodGot)
            {
                moveNextRecorder.mMethod = instance.Type.GetMethod("MoveNext", 0);
                moveNextRecorder.mMethodGot = true;
            }

            if (moveNextRecorder.mMethod != null)
            {
                return (bool)appdomain.Invoke(moveNextRecorder.mMethod, instance, null);
            }
            else
            {
                return false;
        ...
        AdaptorMethodRecorder resetRecorder = new AdaptorMethodRecorder();
        public void Reset()
        {
            if (!resetRecorder.mMethodGot)
            {
                resetRecorder.mMethod = instance.Type.GetMethod("Reset", 0);
                resetRecorder.mMethodGot = true;
            }

            if (resetRecorder.mMethod != null)
            {
                appdomain.Invoke(resetRecorder.mMethod, instance, null);
            }
        }
