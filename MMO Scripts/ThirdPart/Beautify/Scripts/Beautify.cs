    ...
    public class Beautify : MonoBehaviour {
        ...
        Material bMat;
								...
								protected virtual void OnRenderImage (RenderTexture source, RenderTexture destination) {
												...
												if (renderPass == 0 || doFinalBlur) {
																if (doFinalBlur) {
																				...
																				float aspectRatio = (float)currentCamera.pixelHeight / currentCamera.pixelWidth;
																				rtBlurTex = RenderTexture.GetTemporary (size, Mathf.Max (1, (int)(size * aspectRatio)), 0, rtFormat);
																				if (renderPass == 0) {
																								rtBeauty = RenderTexture.GetTemporary (rtBlurTex.width, rtBlurTex.height, 0, rtFormat);
																				}
																} else {
																				rtBeauty = RenderTexture.GetTemporary (source.width, source.height, 0, rtFormat);
																}
												}
												...
												if (_depthOfField && allowExtraEffects) {
																...
																if (Camera.current != sceneCamera) {
																				...
																				rtDoF = RenderTexture.GetTemporary (source.width / _depthOfFieldDownsampling, source.height / _depthOfFieldDownsampling, 0, rtFormat);
								...
								void BlurThisDownscaling (RenderTexture rt, RenderTexture downscaled, float blurScale = 1f) {
												RenderTexture rt2 = RenderTexture.GetTemporary (rt.width, rt.height, 0, rtFormat);
												rt2.filterMode = FilterMode.Bilinear;
												float ratio = rt.width / downscaled.width;
